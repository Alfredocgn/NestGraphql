...
export class ListsService {
  ...
  async findAll(
    ...
    const queryBuilder = this.listsRepository.createQueryBuilder().take(limit).skip(offset).where(`"userId" = :userId`,{userId:user.id})
  ...
  async findOne(id: string,user:User) : Promise<List> {
    ...
    if(!list){
      throw new NotFoundException(`The list with the id: ${id} is not found`)
    }
    return list
  }
